...
class ImprovedEmbeddingGuard:
    ...
    def __init__(self, model_name='all-MiniLM-L6-v2', verbose=False,
             pattern_threshold=0.3, jailbreak_boost=1.5, benign_penalty=0.6,
             attack_fallback=0.15, confidence_threshold=0.4):
        ...
        self.jailbreak_boost = jailbreak_boost
        self.benign_penalty = benign_penalty
        self.attack_fallback = attack_fallback
    ...
    def check(self, text):
        ...
        if pattern_score > self.pattern_threshold:  # Use instance variable
        ...
        scores_by_type["jailbreak"] = scores_by_type.get("jailbreak", 0.0) * self.jailbreak_boost  # Use instance variable
        
        # Step 4: Penalize benign score to avoid false negatives
        scores_by_type["benign"] = scores_by_type.get("benign", 0.0) * self.benign_penalty  # Use instance variable
        ...
        if confidence < self.confidence_threshold:  # Use instance variable
            # Try to find any attack signal
            attack_scores = {k: v for k, v in scores_by_type.items() if k != "benign"}
            if attack_scores and max(attack_scores.values()) > self.attack_fallback:  # Use instance variable
